...
INSERT OR IGNORE INTO users_new (id, username, email, password, created_at, updated_at)
SELECT id, username, email, password, created_at, updated_at
FROM users WHERE EXISTS (
  SELECT 1 FROM pragma_table_info('users')
  WHERE name IN ('id','username','email','password','created_at','updated_at')
);
...
INSERT OR IGNORE INTO deployments_new 
...
SELECT id, user_id, project_id, commit_hash, action, status, started_at, finished_at,
       log_output, artifact_path, COALESCE(pipeline_stage_uuid, NULL)
FROM deployments;
...
INSERT OR IGNORE INTO activity_logs_new 
  (id, project_id, deployment_id, action, message, created_at)
